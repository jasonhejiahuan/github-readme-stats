...
const fetchStats = async (
  ...
  let res = await statsFetcher({
    ...
    startTime: commits_year
      ? `${commits_year}-01-01T00:00:00Z`
      : undefined,
    env,
  });
  ...
  if (include_all_commits) {
    stats.totalCommits =
      await totalCommitsFetcher(username, env);
  } else {
    stats.totalCommits =
  ...
  stats.totalIssues =
    user.openIssues.totalCount +
    user.closedIssues.totalCount;

  if (include_discussions) {
    stats.totalDiscussionsStarted =
      user.repositoryDiscussions.totalCount;
  }
